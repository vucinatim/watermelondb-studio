...
exports.DbServerManager = void 0;
var _watermelondb = require("@nozbe/watermelondb");
...
var _executeSql = require("./utils/execute-sql");
...
const createHttpResponse = (body, statusCode = 200, statusMessage = 'OK') => {
  const bodyString = JSON.stringify(body);
  return [`HTTP/1.1 ${statusCode} ${statusMessage}`, 'Content-Type: application/json; charset=utf-8', `Content-Length: ${_buffer.Buffer.byteLength(bodyString, 'utf8')}`, `Access-Control-Allow-Origin: ${VIEWER_ORIGIN}`, 'Connection: close', '', bodyString].join('\r\n');
};
const createCorsResponse = () => {
  return ['HTTP/1.1 204 No Content', `Access-Control-Allow-Origin: ${VIEWER_ORIGIN}`, 'Access-Control-Allow-Methods: GET, POST, OPTIONS', 'Access-Control-Allow-Headers: Content-Type, Last-Event-ID', 'Connection: close', '', ''].join('\r\n');
};
// #endregion
...
class DbServerManager {
  ...
  handleDbRequest = async lastEventId => {
    ...
    for (const tableName of tableNames) {
      ...
      if (lastEventId && hasLastModified) {
        query = collection.query(_watermelondb.Q.where('last_modified', _watermelondb.Q.gt(lastEventId)));
      }
      const records = await query.fetch();
      const rawRecords = records.map(record => {
        if (hasLastModified) {
          const recordLastModified = record._raw.last_modified;
  ...
  handleFullDbRequest = async () => {
    const {
      data
    } = await this.handleDbRequest();
    return createHttpResponse({
      data
    });
  };
  handleExecuteSqlRequest = async request => {
    const {
      body
    } = request;
    let requestData = {};
    ...
    const results = await (0, _executeSql.executeSql)(this.database, sql, params);
    return createHttpResponse({
      ok: true,
      results
    });
  };
  handleDeviceInfoRequest = async () => {
    return createHttpResponse({
      name: Device.modelName,
      os: _reactNative.Platform.OS
    });
  };
  handleHeartbeatRequest = async () => {
    return createHttpResponse({
      status: 'ok',
      timestamp: Date.now()
    });
  ...
  startDbObserver = () => {
    ...
    const validCollections = allTableNames.map(tableName => ({
      tableName,
      ...
      if (!collection) {
        log(`[Warning] No collection found for table "${tableName}" in schema. Skipping observer.`);
        return false;
      }
      return true;
    });
    ...
    const subscription = this.database.withChangesForTables(tableNames).subscribe(changes => {
      ...
      for (const change of changes) {
        ...
        if (tableSchema.columns.last_modified) {
          const recordLastModified = change.record._raw.last_modified;
          if (recordLastModified && recordLastModified > maxLastModified) {
            maxLastModified = recordLastModified;
          }
        }
      ...
      const message = `id: ${eventId}\nevent: db_change\ndata: ${JSON.stringify(sanitizedChanges)}\n\n`;
  ...
  attemptToStart = port => {
    ...
    const newServer = _reactNativeTcpSocket.default.createServer(socket => {
      socket.on('data', async data => {
        try {
          ...
          if (url.pathname === '/events') {
            ...
            log(`SSE client connected. ${lastEventIdStr ? `Last ID: ${lastEventIdStr}` : 'New client.'}`);
            const headers = ['HTTP/1.1 200 OK', 'Content-Type: text/event-stream', 'Connection: keep-alive', 'Cache-Control: no-cache', `Access-Control-Allow-Origin: ${VIEWER_ORIGIN}`, '', ''].join('\r\n');
    ...
    newServer.on('error', e => {
      ...
      if (e.code === 'EADDRINUSE') {
        log(`Port ${port} is in use. Closing this instance and trying next port...`);
        this.isClosing = true;
        newServer.close(() => {
...
exports.DbServerManager = DbServerManager;
