...
const createHttpResponse = (body, statusCode = 200, statusMessage = 'OK') => {
  ...
  return [`HTTP/1.1 ${statusCode} ${statusMessage}`, 'Content-Type: application/json; charset=utf-8', `Content-Length: ${Buffer.byteLength(bodyString, 'utf8')}`, `Access-Control-Allow-Origin: ${VIEWER_ORIGIN}`, 'Connection: close', '', bodyString].join('\r\n');
};
